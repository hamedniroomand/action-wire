...
navigation:
  title: Headless bridge
---

# Headless bridge

> `createAgentBridge` from `@action-wire/agent`. The whole product, without a user interface.

Use this when your application renders its own chat surface.

```ts [assistant.ts]
import { createAgentBridge, openAICompatible } from '@action-wire/agent';
