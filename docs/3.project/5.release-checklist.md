...
The scope `@action-wire` is not an npm org until someone creates it. Create the org, then publish. The workspace root is also named `action-wire` and is `"private": true`. Publish from `packages/*` only. Public version is **0.1.0**.
...
| Tarball                        | Bytes |
| ------------------------------ | ----- |
| `action-wire-core-0.1.0.tgz`   | 7325  |
| `action-wire-webmcp-0.1.0.tgz` | 8857  |
| `action-wire-agent-0.1.0.tgz`  | 10611 |
| `action-wire-0.1.0.tgz`        | 13241 |
...
1. `npm login`
2. Create the `@action-wire` org on npm if it does not exist.
3. Build: `pnpm --filter ./packages/core --filter ./packages/webmcp --filter ./packages/agent --filter ./packages/widget build`
4. Publish in order, from each package directory: `@action-wire/core`, `@action-wire/webmcp`, `@action-wire/agent`, `action-wire`.
