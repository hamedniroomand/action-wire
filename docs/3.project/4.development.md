...
| `packages/agent`           | Bridge, session, confirmation policy, OpenAI-compatible adapter. |
| `packages/widget`          | The Web Component, published as `action-wire`.                   |
...
- No React, Vue, Svelte, or voice dependency in `packages/core`,
  `packages/webmcp`, `packages/agent`, or `packages/widget`.
...
Pages are Markdown. The directory name sets the section, and the number prefix
sets the order; the prefix is removed from the URL, so
`1.guide/4.quickstart.md` is served at `/guide/quickstart`. Site options live in
