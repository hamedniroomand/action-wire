...
This host registers a native WebMCP tool and mounts `createAssistant` with `onMount`. `onDestroy` disposes the assistant and aborts the registration signal. The handler reads current reactive state. Imports use the same built package output as the Vanilla, React, and Vue examples.
...
Open http://127.0.0.1:4179 in flagged Chromium.
